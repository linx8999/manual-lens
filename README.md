...
左侧是手册目录树（本例 18 个章节节点，真实手册可达 1300+ 节点），右侧渲染真实手册页并高亮引用片段；页面正文已做模糊处理以规避版权。

![内置阅读器](docs/images/outline-and-reader.png)
...
这一节说明这个智能体在实现上做过的关键优化和取舍，方便判断它是否适合你的场景，也方便二次开发。

### 检索：中文为什么不能用默认分词

SQLite FTS5 的 `unicode61` 会把连续汉字当成一个 token，所以必须自己切词。早期实现同时索引单字和双字，结果单字（低/功/模/式）几乎每页都有，BM25 的 IDF 被拉平，排序基本失效——现在**只索引双字词**，英文与寄存器名原样索引。

| 优化 | 解决的问题 |
|---|---|
| 中文双字词索引 | 单字噪声淹没 BM25 排名 |
| 疑问词剥离（有哪几种 / 怎么 / 区别…） | 「有哪几种」在手册里极罕见，IDF 反而把它当强信号，把正确页挤掉 |
| 文档频率过滤（>30% 的词剔除） | 页眉页脚、章名每页重复，是噪声不是信号 |
| 编号小节加权（`4.3 低功耗模式`） | 技术手册里「编号 + 标题」是最强的定位信号 |
| 目录定向 | 关键词命中目录节点时，该章节起始页权重更高 |
| 多路查询 + RRF | 整句 / 关键词 / 单词 / 领域词各走一路，关键词权重更高（1.25 vs 0.9） |

实测：手写真实问法 51/51，目录自动生成章节问题 41/44，合计 **92/95 = 96.8%**。

### 范围过滤：三条链路同步收敛

勾选资料不只是过滤最终结果，而是**词法检索、向量检索、目录定向三处都加 `document_id` 约束**，连 IDF 的文档频率也只在勾选范围内统计。实测勾选后越界结果 **0 条**。

### 图片定位：从「能跑」到 96%

- **候选选择**：一开始用 FTS 预筛候选页，但 FTS 按「命中块数」排序，真正的页面常被挤出候选池——改成在勾选范围内**全量逐页评分**。
- **打分**：从「关键词覆盖率」改成 **IDF 加权**（`idf²`）并剔除满库样板词，通用词不再左右判断。
- **编码坑**：PowerShell 管道默认用控制台代码页输出，OCR 出的中文到 Node 侧全是乱码（只有英文正常），准确率一度只有 **4.2%**——强制两端 UTF-8 后修复。
- **双引擎**：视觉模型（默认，识别率高）失败时**自动回落**本机 Windows OCR（离线、截图不出本机）。
- **token 控制**：截图统一压到长边约 1200px 再发送，一次定位约 1000–1500 tokens。

实测：50 张不同裁切 / 压缩比的截图，视觉引擎 **48/50 = 96.0%**，本机 OCR 45/50 = 90.0%。

### 阅读器：拖动以前为什么卡

拖动分割条时列宽每帧都在变，早期实现每帧都触发 React 重渲染 + 重排 700+ 页面槽 + 重新光栅化可见 PDF 页。现在三层解决：

1. 拖动期间只通过 **rAF 写 CSS 变量**，不触发 setState；
2. 只做**合成层 transform 预览**，松手才真正重新渲染画布；
3. 页面槽 `contain: layout paint`，长列表布局隔离，拖动时关闭阴影重绘与悬停命中测试。

### 索引自愈

分词器或章节标题规则升级后，老库**不需要重新导入手册**：启动时比对 `schema_meta.search_index_version`，版本落后就用现有正文重算 `search_tokens` 与 `heading`（4162 个文本块约 **1.9 秒**）。

### 结论可复现

仓库带了一套基准脚本（解析覆盖率 / 检索命中率 / 图片定位准确率 / 范围过滤 / 索引迁移），改完算法直接跑出前后对比，而不是凭感觉判断。
